            ...
            <div class="flex justify-between items-center bg-white rounded-lg shadow-sm border border-slate-200 p-4 mb-6">
                <div class="flex items-center space-x-3">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <?php $autoRefreshDefault = !$data['Game']['IsComplete'] ? 'checked' : ''; ?>
            ...
            <div class="text-center flex flex-col justify-center border-x border-slate-100 px-4">
                ...
                <div class="inline-block bg-slate-900 text-white px-4 py-1 rounded-full text-sm font-bold tracking-wide mx-auto mt-2">
                    <?php if ($game['IsComplete']): ?>
                        FINAL
                    <?php else: ?>
                        P<?= $game['CurrentPeriod'] ?> | <?= floor($game['ClockSecondsRemaining'] / 60) ?>:<?= str_pad($game['ClockSecondsRemaining'] % 60, 2, '0', STR_PAD_LEFT) ?>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
                ...
                <table class="w-full text-sm text-center">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <?php 
                            $compareStats = ['ShotsOnGoal' => 'Shots on Goal', 'PowerPlayEfficiency' => 'Power Play', 'PenaltyMinutes' => 'Penalty Minutes', 'FaceoffsWon' => 'Faceoffs Won'];
                        ?>
                        <?php foreach ($compareStats as $key => $label): ?>
                        ...
                        <table class="w-full text-sm text-left">
                            <thead class="bg-slate-50 text-slate-500 text-xs uppercase border-b border-slate-200">
                                <tr>
                                    ...
                                    <th class="px-3 py-3 font-semibold text-center">G</th>
                                    <th class="px-3 py-3 font-semibold text-center">A</th>
                                    <th class="px-3 py-3 font-semibold text-center">PTS</th>
                                    <th class="px-3 py-3 font-semibold text-center">SOG</th>
                                    <th class="px-3 py-3 font-semibold text-center">+/-</th>
                                    <th class="px-3 py-3 font-semibold text-center">PIM</th>
                                    <th class="px-3 py-3 font-semibold text-center">FOW-L</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-100">
                                <?php if (isset($stats[$teamKey]['Skaters'])): ?>
                                    <?php foreach ($stats[$teamKey]['Skaters'] as $skater): ?>
                        ...
                        <table class="w-full text-sm text-left">
                            <thead class="bg-white text-slate-500 text-xs border-b border-slate-200">
                                <tr>
                                    ...
                                    <th class="px-3 py-2 font-semibold text-center">SV</th>
                                    <th class="px-3 py-2 font-semibold text-center">GA</th>
                                    <th class="px-3 py-2 font-semibold text-center">SV%</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-100">
                                <?php if (isset($stats[$teamKey]['Goalies'])): ?>
                                    <?php foreach ($stats[$teamKey]['Goalies'] as $goalie): ?>
                                        <tr class="hover:bg-blue-50/50 transition-colors">
                                            <td class="px-4 py-2 font-medium text-slate-800 whitespace-nowrap">
                ...
                <?php foreach (['Points', 'Goals'] as $leaderCat): ?>
                    ...
                    <ul class="space-y-3">
                        ...
                        <?php foreach ($topLeaders as $leader): ?>
                            <li class="flex justify-between items-center text-sm border-l-2 pl-3 <?= $leader['Team'] == $home['Id'] ? 'border-blue-500' : 'border-slate-800' ?>">
                                <div>
                    ...
                    <?php else: ?>
                        <div class="divide-y divide-slate-100">
                            <?php foreach (array_reverse($plays) as $play): ?>
                                <div class="p-4 hover:bg-slate-50 transition-colors flex gap-4 <?= $play['Type'] === 'Goal' ? 'bg-green-50/50' : '' ?>">
                                    ...
                                    <div>
                                        <div class="flex items-center gap-2 mb-1">
                                            <span class="text-xs font-bold px-2 py-0.5 rounded <?= $play['TeamId'] === $home['Id'] ? 'bg-blue-100 text-blue-800' : 'bg-slate-200 text-slate-800' ?>">
                                                <?= htmlspecialchars($play['TeamId']) ?>
                                            </span>
                                            <span class="text-xs font-semibold uppercase tracking-wider <?= $play['Type'] === 'Goal' ? 'text-green-600' : 'text-slate-500' ?>">
